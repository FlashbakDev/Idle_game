Shader "Custom/Matrix"
{
	Properties
	{
		...
		_SpeedMax ("Speed Max", range(0, 50.)) = 20.
		_SpeedMin ("Speed Min", range(0, 50.)) = 2.
		_Density ("Density", range(0, 500.)) = 5.
		_DirectionDown ("Direction Down", range(-1,1) ) = 1
		_DirectionLeft ("Direction Left", range(-1,1) ) = 0
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f_img i) : SV_Target
			{
				...
				ipos.y += floor( _DirectionDown * _Time.y * max(_SpeedMin, _SpeedMax * noise(ipos.x)));
				ipos.x += floor( _DirectionLeft * _Time.y * max(_SpeedMin, _SpeedMax * noise(ipos.y)));
